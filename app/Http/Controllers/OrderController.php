    ...
    private function cancelOffers($offers, $language, $order_id = 0)
    {
        if (!empty($offers)) {
            foreach ($offers as $offer) {
                ...
                $message = ': ' . (($offersOrderDetail && $offersOrderDetail->from) ? $offersOrderDetail->from->name : '') . ' - ' . (($offersOrderDetail && $offersOrderDetail->to) ? $offersOrderDetail->to->name : '');
    ...
    public function booking(Request $request)
    {
        ...
        if ($request['offer_id'] != '') {
            if ($offer = Offer::where('id', $request['offer_id'])->first()) {
                ...
                return $this->error(translate_api('Sorry, There is no such offer', $language), 400);
            }
    ...
    public function bookingCancel(Request $request)
    {
        ...
        if ($first_offer) {
            ...
            if (!$orderDetail->client) {
                return $this->error(translate_api('No client was found matching the OrderDetail you selected', $language), 400);    
            }
            ...
            if ($old_offer_status == Constants::NEW) {
                ...
                if ($id == $order->driver_id) {
                    $user_id = ($orderDetail->client) ? $orderDetail->client->id : 0;
                    $title = 'Passenger canceled the booking';
                    $device = ($orderDetail->client) ? json_decode($orderDetail->client->device_id) : [];
                } else {
                    $title = 'A passenger has canceled a reservation on your trip';
                    $device = ($order->driver) ? json_decode($order->driver->device_id) : [];
                }
            }
        ...
        if (isset($options) && count($options) > 0) {
            foreach ($options as $option) {
                $data[] = [
                    'id' => $option->id,
                    'name' => $option->name,
                    'icon' => ($option->icon) ? "http://admin.easygo.uz/storage/option/" . $option->icon : '',
                ];
            }
